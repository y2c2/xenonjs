        ...
        int lineno)
{
    ...
    program = xjs_parser_start(&err, tokens, xjs_true);
    if (program == NULL)
    {
...
static void test_c0m_simple(struct cunittest *this_cu, \
        const char *test_file)
{
    CUNITTEST_DECLARE(this_cu);

#define test_c0m_item_ok(_this_cu, _expected_result, _source, _filename, _lineno) \
    _test_c0m_item_ok(_this_cu, _expected_result, _source, _filename, _lineno)
...
#define EXPECTED_LINE_MAX 8192
    {
        ...
        for (;;)
        {
            ...
            /* Perform testing */
            test_c0m_item_ok(CUNITTEST_THIS, expected, testcase, test_file, cur_lineno);
        }
...
void test_c0m(const char *test_file)
{
    CUNITTEST_HOLD_AND_DECLARE();
    CUNITTEST_INIT_WITH_TITLE("c0m");

    if (test_file == NULL)
    {
        fprintf(stderr, "error: c0m testcase missing\n");
        fflush(stderr);
        return;
    }
    test_c0m_simple(CUNITTEST_THIS, test_file);
